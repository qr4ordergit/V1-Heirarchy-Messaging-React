...
const useScheduleMediaDecryptor = () => {
    ...
    return async function (msgs: SCHEDULED_MESSAGE[]): Promise<SCHEDULED_MESSAGE[]> {
        if (msgs.length === 0) {
            return [];
        }

        const updatedMessages: SCHEDULED_MESSAGE[] = [];
        ...
        for (let i = 0; i < msgs.length; i++) {
            ...
            const medias = message.media_url;
            ...
            const modifiedMsg: SCHEDULED_MESSAGE = {
                ...message,
...
export default useScheduleMediaDecryptor;
